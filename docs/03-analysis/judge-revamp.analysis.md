...
> 주의: 이 측정은 2026-07 시점 값으로, 이후 채점 모델이 gpt-5-mini로 바뀌고
> 분야1 프롬프트에 점수 구간이 추가되어 현재 값은 재현되지 않습니다.

## 3. 남은 관찰 사항 (후속 사이클 후보)

1. ~~**분야1 점수대 앵커**~~ — 완료. 기관용 프롬프트에 4구간 기준을 작성해
   항목별 점수 폭이 최대 35 → 8로 축소, 판정 불안정 0건
2. **rationale 노출**: 분야별 채점 근거 텍스트를 결과서에 표시 (`representativeSampleIndex` 활용)
3. ~~**구식 문서 정리**~~ — 완료. `설명/`은 현재 서비스 기준으로 전면 재작성,
   `assets/`·`.stitch/`·`github-upload/`·구 회귀 기준값은 삭제
